(function() {
	...
	var jqueryAlert = function(opts) {
		...
		dialog.framework = function() {
			...
			dialog.buttonsLength = dialog.buttons.length;$container.append($title).append($content);
			if (option.style == "pc") {
				$container.append($closeBtn).addClass("pcAlert")
			}
			if (option.modal || option.modal == "true") {
				$("body").append($modal);option.bodyScroll && $("body").css("overflow", "hidden")
			}
			$("body").append($container);$content.css({
				"text-align" : option.contentTextAlign
			});
			if (parseInt(option.minWidth) > parseInt($container.css("width"))) {
				option.width = option.minWidth
			}
			$modal.css("position", option.position);$modal.css("z-index", zIndex);++zIndex;
			...
			$container.css("width", option.width);$container.css("height", option.height);
			...
			if (dialog.buttonsLength > 0) {
				$container.append($buttonBox);$content.css("padding-bottom", "46px");
		...
		function isSelfClose() {
			if (dialog.buttonsLength <= 0 && option.style != "pc") {
				setTimeout(function() {
					$container.fadeOut(300);$modal.fadeOut(300);option.bodyScroll && $("body").css("overflow", "auto")
				}, option.closeTime)
			}
		}
		...
		dialog.close = function() {
			dialog.toggleAnimate().fadeOut(dialog.time);$modal.fadeOut(dialog.time);option.bodyScroll && $("body").css("overflow", "auto")
		};option.style == "pc" && $closeBtn.bind(dialog.event, dialog.close);option.isModalClose && $modal.bind(dialog.event, dialog.close);
		dialog.destroy = function() {
			dialog.toggleAnimate().fadeOut(dialog.time);setTimeout(function() {
				$container.remove();$modal.remove();option.bodyScroll && $("body").css("overflow", "auto")
			}, dialog.time)
		};
		dialog.show = function() {
			$modal.css("z-index", zIndex);++zIndex;$container.css({
				"z-index" : zIndex,
			});
			if (option.animateType == "scale") {
			...
			$modal.fadeIn();option.bodyScroll && option.modal && $("body").css("overflow", "hidden");
			isSelfClose()
